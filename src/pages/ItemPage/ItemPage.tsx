...
export const ItemPage: React.FC = () => {
  ...
  const dataItems:any = useSelector((state: any) => state.goods.goodsArray);
   const fullRecipe:any = dataItems?.find(elem => elem?.value?.id === id);
  ...
  return (
    <div className={style.itemPage}>
        ...
        <div className={style.itemPage__rightBox}>
          ...
          <div className={style.itemPage__boxs}>
            ...
            <div className={style.itemPage__coverLine}>
              ...
              <p className={style.itemPage__text}>{fullRecipe?.value?.season}</p>
            </div>
            <div className={style.itemPage__coverLine}>
              <p className={style.itemPage__name}>Состав материала</p>
              <p className={style.itemPage__text}>{fullRecipe?.value?.compound}</p>
            </div>
            <div className={style.itemPage__coverLine}>
              <p className={style.itemPage__name}>Тип</p>
              <p className={style.itemPage__text}>{fullRecipe?.value?.type}</p>
            </div>
            <div className={style.itemPage__cover}>
              <p className={style.itemPage__price}>Цена {fullRecipe?.value?.price} уе </p>
              <ButtonContrast
                imageButton={bag}
      ...
      <div className={style.itemPage__bottomBox}>
          ...
          {buttonClick === 0 && <p className={style.itemPage__buttonText}>{fullRecipe?.value?.mainDescription} </p>}
          {buttonClick === 1 && <p className={style.itemPage__buttonText}>{fullRecipe?.value?.compound} </p>}
          {buttonClick === 2 && <p className={style.itemPage__buttonText}>{fullRecipe?.value?.other} </p>}
      </div>
      <BoxPopularItems/>
    </div>
  );
